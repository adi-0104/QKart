...
const AddNewAddressView = ({
  ...
  const addHandler = async () => {
   ...
   //set newAddress variables to default 
    handleNewAddress({
...
const Checkout = () => {
  ...
  const deleteAddress = async (token, addressId) => {
    ...
    try {
      ...
      // console.log(`deleted ${addressId}: ${response.data}`);
  ...
  const validateRequest = (items, addresses) => {
    ...
    if(items.length === 0){
      enqueueSnackbar("You do not have any items in cart", {variant: "warning"});
      return false;
    }
    
    // Not enough balance available to checkout cart item - "You do not have enough balance in your wallet for this purchase"
    ...
    // No addresses added for uset - "Please add a new address before proceeding."
    else if(addresses.all.length === 0){
      enqueueSnackbar("Please add a new address before proceeding.", {variant: "warning"});
      return false
    }

    // No address selected for checkout - "Please select one shipping address to proceed."
  ...
  const orderHandler = async () => {
    const success = await performCheckout(token, items, addresses);
    if(success){
      enqueueSnackbar("Order placed successfully", {variant: "success"})
      let newBalance = balance - getTotalCartValue(items);
          ...
          <Box className="shipping-container" minHeight="100vh">
            ...
            <Box>
              ...
              {addresses.all.length ? (
                <Stack spacing={0}>
                  {addresses.all.map((currAdd) => (
                    <Box
                      key={currAdd._id}
                      onClick={() =>
                        setAddresses({
                          ...addresses,
                          selected: `${currAdd._id}`,
                        })
                      }
                      className={
                        addresses.selected === currAdd._id
                          ? "address-item selected"
                          : "address-item not-selected"
                      }
                    >
                      <Typography my="1rem">{currAdd.address}</Typography>
                      <Button
                        startIcon={<Delete />}
                        variant="text"
                        color="primary"
                        onClick={() => deleteAddress(token, currAdd._id)}
                      >
                        DELETE
                      </Button>
                    </Box>
            ...
            <Button startIcon={<CreditCard />} variant="contained" onClick={orderHandler}>
              PLACE ORDER
            </Button>
          </Box>
        </Grid>
